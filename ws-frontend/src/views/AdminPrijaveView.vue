<template>
  ...
  <div class="card-deck">
    <div v-for="prijava in prijave" class="card " style="width: 18rem;">
      ...
      <div class="card-body div-recenzija" >
        ...
        <a href="#" class="btn btn-primary izmeni" v-on:click="odbaciPrijavu(prijava.id, prijava.razlogOdbijanja)">Odbaci prijavu</a>
        <input type="text" class="form-control" id="inputRazlogPrijave" placeholder="Razlog odbijanja" v-model="prijava.razlogOdbijanja">
 ...
 export default {
   ...
   methods: {
    ...
    odbaciPrijavu: function(id, razlogOdbijanja){
        axios
        .post(`http://localhost:8081/api/rejectreport/${id}?logovaniUsername=`+this.$route.params.username+`&reason=`+ razlogOdbijanja, {
        withCredentials: true,
            })
            .then((res) => {
